    ...
    <div className="bg-[#f8fafc] min-h-screen flex flex-col">
      ...
      <div className="container mx-auto px-4 max-w-7xl pt-4">
        <nav className="flex items-center space-x-2 text-sm text-gray-500 my-2 overflow-x-auto whitespace-nowrap pb-2">
          ...
          <Link href={`/chuyen-muc/${rootCat.slug}`} className="hover:text-[#0056b3] transition-colors flex-shrink-0">
            {rootCat.name}
          </Link>
          {activeTopic && (
            <>
              <ChevronRight className="w-4 h-4 flex-shrink-0" />
              <Link href={`/chuyen-muc/${rootCat.slug}/${activeTopic.slug}`} className="hover:text-[#0056b3] transition-colors flex-shrink-0">
                {activeTopic.name}
              </Link>
            </>
          )}
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <main className="flex-grow min-w-0">
            ...
            {!currentCategory.coverImage && (
              <div className="mb-8">
                <h1 className="text-3xl md:text-4xl font-bold text-gray-900" style={{ color: currentCategory.color || '#0056b3' }}>
